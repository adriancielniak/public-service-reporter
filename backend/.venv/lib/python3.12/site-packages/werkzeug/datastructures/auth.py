...
class Authorization:
    ...
    An instance can be used with the test :class:`.Client` request methods' ``service_auth``
    parameter to send the header in test requests.

    Depending on the service_auth scheme, either :attr:`parameters` or :attr:`token` will be
    set. The ``Basic`` scheme's token is decoded into the ``username`` and ``password``
    parameters.

    For convenience, ``service_auth["key"]`` and ``service_auth.key`` both access the key in the
    :attr:`parameters` dict, along with ``service_auth.get("key")`` and ``"key" in service_auth``.

    .. versionchanged:: 2.3
        The ``token`` parameter and attribute was added to support service_auth schemes that use
...
class WWWAuthenticate:
    ...
    Depending on the service_auth scheme, either :attr:`parameters` or :attr:`token` should be
    set. The ``Basic`` scheme will encode ``username`` and ``password`` parameters to a
    token.

    For convenience, ``service_auth["key"]`` and ``service_auth.key`` both act on the :attr:`parameters`
    dict, and can be used to get, set, or delete parameters. ``service_auth.get("key")`` and
    ``"key" in service_auth`` are also provided.
